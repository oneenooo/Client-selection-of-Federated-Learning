...
class BiasLoader(Loader):
    ...
    def get_partition(self, partition_size, pref,client):
        # Get a non-uniform partition with a preference bias

        # Extract bias configuration from config
        if client.client_id < 33:
            bias = self.config.data.bias['primary1']
        else:
            if client.client_id < 66:
                bias = self.config.data.bias['primary2']
            else:
                bias = self.config.data.bias['primary3']
