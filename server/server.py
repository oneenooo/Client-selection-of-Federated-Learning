...
class Server(object):
    ...
    def run(self):
        ...
        print("total choose record:{}".format(self.select_record))
        if reports_path:
            with open(reports_path, 'wb') as f:
                pickle.dump(self.saved_reports, f)
            logging.info('Saved reports: {}'.format(reports_path))

        print("total {} round make the gobal model accuracy {}".format(self.config.fl.rounds,accuracy0))
    ...
    def round(self):
        ...
        for client in sample_clients:
            self.select_record[client.client_id]+=1
        ...
        if self.config.clients.do_test:  # Get average accuracy from client reports
            ...
            accuracy = fl_model.test(self.model.to(self.config.fl.device), testloader,self.config.fl.device)
    ...
    def extract_client_updates(self, reports):
        ...
        for weight in weights:
            ...
            for i, (name, weight) in enumerate(weight):
                ...
                # Calculate update
                delta = weight.to("cpu") - baseline.to("cpu")
    ...
    def federated_averaging(self, reports):
        ...
        for i, (name, weight) in enumerate(baseline_weights):
            updated_weights.append((name, weight.to("cpu") + avg_update[i]))
    ...
    def set_client_data(self, client):
        ...
        if loader != 'shard':
            if self.config.data.partition.get('size1'):
                if client.client_id < 33:
                    partition_size = self.config.data.partition.get('size1')
                else:
                    if client.client_id < 66:
                        partition_size = self.config.data.partition.get('size2')
                    else:
                        partition_size = self.config.data.partition.get('size3')
        ...
        if loader == 'basic':
            ...
            data = self.loader.get_partition(partition_size, client.pref,client)
        elif loader == 'shard':
    ...
    def save_model(self, model, path):
        path += '/global_'+self.config.algorithm+"_"+str(self.config.clients.per_round)+"_"+str(self.config.data.IID)
        torch.save(model.state_dict(), path)
        logging.info('Saved global model: {}'.format(path))

    def save_model_client(self, model, path,client):
        path += '/single_'+self.config.algorithm+"_"+str(self.config.clients.per_round)+"_"+str(self.config.data.IID)+str(client.client_id)
        torch.save(model.state_dict(), path)
        logging.info('Saved global model: {}'.format(path))
